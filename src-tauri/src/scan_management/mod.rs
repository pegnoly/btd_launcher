...
pub async fn scan_files(path_manager: State<'_, PathManager>) -> Result<(),()> {
    ...
    let entries = std::fs::read_dir(path_manager.data()).unwrap();
    ...
    let zip_file = std::fs::File::create(
        path_manager.data().join("MCCS_GeneratedFiles.pak")
    ).unwrap();
    let mut map_zipped = zip::ZipWriter::new(zip_file);
    map_zipped.start_file("scripts/generated/creatures.lua", Default::default()).unwrap();
    map_zipped.write_all(creatures_generated_file.as_bytes()).unwrap();
    map_zipped.start_file("scripts/generated/heroes.lua", Default::default()).unwrap();
    map_zipped.write_all(heroes_generated_file.as_bytes()).unwrap();
    map_zipped.start_file("scripts/generated/artifacts.lua", Default::default()).unwrap();
    map_zipped.write_all(artifacts_generated_file.as_bytes()).unwrap();
    map_zipped.start_file("scripts/generated/spells.lua", Default::default()).unwrap();
    map_zipped.write_all(spells_generated_file.as_bytes()).unwrap();
